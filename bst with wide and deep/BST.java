...
class BST<T> {
	...
	public ArrayList<BSTNode> DeepAllNodes(int order) {
		...
		if (Root != null) {
			return DeepAllNodes(Root, order);
		}
	...
	private ArrayList<BSTNode> DeepAllNodes(BSTNode node, int order) {
		
		ArrayList<BSTNode> nodeFlatList = new ArrayList<BSTNode>();
		...
		if (order == 2) {
			nodeFlatList.add(node);
		}
		
		if (node.LeftChild != null) {
			nodeFlatList.addAll(DeepAllNodes(node.LeftChild, order));
		}
		...
		if (node.RightChild != null) {
			nodeFlatList.addAll(DeepAllNodes(node.RightChild, order));
		}
